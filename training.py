...
for epoch in range(epochs):
    ...
    for X_train, y_train in tqdm.tqdm(train_loader):
        ...
        loss = criterion(outputs[3], y_train[3])
    ...
    with torch.no_grad():
        for X_val, y_val in val_loader:
            ...
            outputs = model(X_val)
            loss = criterion(outputs[3], y_val[3])
            val_loss += loss.item()
...
# 儲存模型
torch.save(model.state_dict(), 'x_lstm_model_state_dict.pth')
logging.info("模型已成功儲存！")
# --- 最終測試 ---
model.eval()
with torch.no_grad():
    total_test_loss = 0
    for X_test, y_test in test_loader:
        ...
        loss = criterion(outputs[3], y_test[3])
